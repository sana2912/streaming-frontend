                                ...
                                list_data.map((list, idx) => {
                                    return (
                                        <div key={idx} className="flex justify-between my-2">
                                            <label className="text-black text-xl self-center" htmlFor={`playlist${idx}`}>{list.lists}</label>
                                            <input className="w-6 h-6 self-center" data-state={list.state} data-list_id={list.list_id} type="checkbox" name={list.lists} id={`playlist${idx}`} defaultChecked={list.playlist_state} />
                                        </div>
                                    )
                                })
                            }
                            <div className="flex justify-between mt-4">
                                <button onClick={() => set_list_hiding(true)} className=" px-2 py-1 bg-black rounded-full cursor-pointer hover:bg-neutral-800" type="button">ลิสต์ใหม่</button>
                                <button className=" px-2 py-1 bg-black rounded-full cursor-pointer hover:bg-neutral-800" type="submit">บันทึก</button>
                            </div>
                        </form>
                        : <div className="flex flex-col bg-white p-6">
                            {same ? <p className="text-red-600 text-[12px] m-2">ชื่อเพลย์ลิตส์ของคุณซ้ำกัน</p> : <></>}
                            <input onChange={(e) => set_listname(e.target.value)} className='border-1 border-neutral-500 rounded-[18px] px-2 py-1 text-neutral-500 focus:outline-none focus:border-1' type="text" name="newlist" id="newlist" placeholder="ชื่อเพลย์ลิตส์" />
                            <div className="flex justify-between mt-4">
                                <button onClick={() => set_list_hiding(false)} className=" px-2 py-1 bg-black rounded-full cursor-pointer hover:bg-neutral-800" type="button">ยกเลิก</button>
                                <button onClick={add_new_list} className=" px-2 py-1 bg-black rounded-full cursor-pointer hover:bg-neutral-800" type="submit">สร้าง</button>
                            </div>
                        </div>
                    }
                </div >
