...
const Display = () => {
    ...
    useEffect(() => {
        if (on_album) {
            baground_ref.current.style.background = `linear-gradient(${get_album(album_data, albumId)}, black)`;
        }
        else {
    ...
    return (
        <div ref={baground_ref} className=" h-[100%]">
            <Nav />
            <Routes>
                {/* setting rout to home component*/}
