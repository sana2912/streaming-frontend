...
const Login = ({ set_state }) => {
    ...
    return (
        <div className="flex flex-col relative">
            ...
            <form onSubmit={(e) => onSubmit_func(e)} className="mt-8 flex flex-col gap-6">
                <input onChange={(e) => set_email(e.target.value)} className="border-2 border-neutral-200 text-white px-4 py-1 focus:outline-none" type="text" name="user_name" id="user_name" placeholder="ยูสเซอร์เนม" />
                <input onChange={(e) => set_password(e.target.value)} className="border-2 border-neutral-200 text-white px-4 py-1 focus:outline-none" type="password" name="password" id="user_name" placeholder="รหัสผ่าน" />
                <button className=" px-4 py-1 bg-neutral-600 text-white hover:bg-neutral-800" type="submit">เข้าสู่ระบบ</button>
            </form>
